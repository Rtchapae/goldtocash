...
class ListOrdersAction
{
    ...
    public function execute(
        ...
        if ($orderType) {
            if ($orderType === 'online') {
                $query->where(function ($q) {
                    $q->where('orders.order_type', 'online')
        ...
        if ($nameQuery) {
            $query->where('users.name', 'LIKE', '%' . $nameQuery . '%');
        }
    ...
    private function applySorting(Builder $query, ?string $orderBy, string $orderDir): void
    {
        ...
        } elseif ($orderBy === 'branch') {
            $query->leftJoin('branches', 'orders.branch_id', '=', 'branches.id')
                ->orderBy('branches.display_name', $direction)
                ->select('orders.*');
        } else {
    ...
    {
        $filterOptions = $this->traceRepository->getFilterOptions();
        ...
        $this->availableBranches = \App\Domain\Users\Models\Branch::select('id', 'display_name', 'name')
